...
public class Trait {
	...
	public static bool IsCombatTraitConditionMet(TraitCondition tc) {
		switch(tc) {
		case TraitCondition.Alone: return (CombatManager.combatManager.GetFactionCount("Blob", false) == 1);
		case TraitCondition.ForEachOtherBlob: return (CombatManager.combatManager.GetFactionCount("Blob", false) > 1);
		}
		return false;
	}
